...
echo "$($PSQL "SELECT name FROM (SELECT * FROM games JOIN teams ON games.winner_id = teams.team_id UNION SELECT * FROM games JOIN teams ON games.opponent_id = teams.team_id) AS all_teams WHERE year=2014 AND round='Eighth-Final' ORDER BY name;")"
